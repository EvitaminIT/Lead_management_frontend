...
import ArchiveBtnImg from '../../Image/ArchiveBtn.png'
import Image from 'next/image';



export default function ArchiveUserTable() {
  ...
  const table_coll = useSelector((state) => state.GetArchiveUsersReducer.data);
  const loading = useSelector((state) => state.GetArchiveUsersReducer.loading);
              ...
              {TBLdata.map(({ employee_id, name, designation, department, product ,email_id,employee_status }, index) => {
                ...
                return (
                  <tr key={index}>
                    ...
                    <>
                    <Index.IconButton color='white' size='sm' className='border border-indigo-600'>
                    <Image src={ArchiveBtnImg} />
                    </Index.IconButton>
