...
export default function Create_user({
  ...
  React.useEffect(() => {
    setMarketDroup(MarkerplaceTable.marketplace)
  }, [MarkerplaceTable])
    ...
    <>
      ...
      <div className='p-6 bg-blue-gray-50 h-full'>
                            ...
                            {CommerLoading === "pending" ? <Skeleton /> :
                              <div className='flex justify-center items-center'>
                                {type === "Servicetable_edit" ?
                                  ...
                                  :
                                  <>
                                    <Index.Typography>{MOU}</Index.Typography>
                                    <Index.IconButton
                                      onClick={() => handleRemoveRowClick(index)}
                                      className='bg-transparent shadow-none hover:shadow-none'> <Index.RemoveCircleOutlineOutlinedIcon className='text-black' /> </Index.IconButton>
                                  </>
        ...
        <div className='text-center mt-6'>
          <Index.Button color='green' onClick={() => type === "Servicetable_edit" ? dispatch(UpdateServiceAPI({ accessToken: token.access, data: updateData })) : ""} className='rounded-full'>{`${type === "Servicetable_edit" ? 'Update Service' : 'Create Service'}`}</Index.Button>
        </div>
      </div>
    </>
  )
